...
Do not take on website publishing, repo posting, PR creation, or markdown post editing in this skill. If the user wants the visualization inserted into a site or blog post, use the `storytelling-viz-publish` skill once this skill's artifact is finished.
...
Do not generate publish-ready markdown, PR instructions, or website repo edits in this skill — that is the `storytelling-viz-publish` skill's job, once this skill's artifact is finished.
...
If the user also wants the result published to a Jekyll/GitHub Pages site, that is out of scope here — hand the finished `viz/project-name/` folder to the `storytelling-viz-publish` skill.
